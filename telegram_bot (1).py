...
def extract_clickid_from_url(url):
    ...
    try:
        ...
        # Updated to search for click_id
        if 'click_id' in params: 
            return params['click_id'][0]
        return None
    except:
        return None
    ...
    try:
        ...
        if 'message' in update:
            ...
            # Handle /cancel command
                ...
                users_collection.update_one({'_id': user_id}, {'$set': {'current_mode': None}})
                keyboard = home_keyboard_admin() if user_id == ADMIN_ID else home_keyboard()
                send_message(user_id, "✅ Cancelled. Back to main menu.", reply_markup=keyboard)
            
            # Handle /admin command
            elif text == '/admin' and user_id == ADMIN_ID:
                users_collection.update_one({'_id': user_id}, {'$set': {'current_mode': None}})
                send_message(user_id, "👨‍💼 <b>Admin Panel</b>\n\nSelect an option:", reply_markup=admin_keyboard())
            ...
            # Handle /help command
                ...
                send_message(
                    ...
                    "2. Select an offer\n"
                    "3. Submit your URL with clickid\n"
                    "4. Postbacks will be sent automatically\n\n"
                    "Need support? Use the Help & Support button."
                )
            ...
            # Handle offer mode
                ...
                if not validate_url_format(text, offer['starting_link']):
                    send_message(
                        ...
                        f"Must match: <code>{offer['starting_link']}?clickid=YOUR_ID</code>"
                    )
                    return 'ok', 200
                ...
                if not clickid:
                    send_message(user_id, "❌ Could not extract clickid from URL")
                    return 'ok', 200
                
                # Show processing message
                send_message(user_id, f"⏳ <b>Processing {len(offer['postbacks'])} postbacks...</b>")
            ...
            # Handle offer creation mode
                ...
                try:
                    parts = text.split('|')
                    if len(parts) < 4:
                        send_message(user_id, "❌ Invalid format. Use: Name|StartLink|PB1|PB2|...|D1|D2|...")
                        return 'ok', 200
                    ...
                    if pb_count < 1 or pb_count > 5:
                        send_message(user_id, "❌ Must have 1-5 postbacks")
                        return 'ok', 200
                    ...
                    send_message(user_id, message, reply_markup=admin_keyboard())
            ...
            # Handle offer edit mode
                ...
                try:
                    parts = text.split('|')
                    if len(parts) != 3:
                        send_message(user_id, "❌ Invalid format. Use: OFFER_ID|field|new_value")
                        return 'ok', 200
                    
                    offer_id, field, new_value = [p.strip() for p in parts]
                    
                    if field not in ['name', 'starting_link', 'enabled']:
                        send_message(user_id, "❌ Invalid field. Use: name, starting_link, or enabled")
                        return 'ok', 200
                    
                    updates = {}
                    if field == 'enabled':
                        updates[field] = new_value.lower() in ['true', '1', 'yes', 'enabled']
                    else:
        ...
        # Handle callback queries
            ...
            # Specific offer selected (only match 24-char hex IDs)
            elif callback_data.startswith('offer_') and len(callback_data) == 30:  # 'offer_' (6) + ObjectId (24) = 30
                answer_callback_query(callback_query_id, "")
                offer_id = callback_data.replace('offer_', '')
                
                # Validate it's a proper ObjectId format
                if len(offer_id) == 24 and all(c in '0123456789abcdef' for c in offer_id.lower()):
                    offer = get_offer(offer_id)
                    ...
                    send_message(
                        user_id,
                        f"🎁 <b>{offer['name']}</b>\n\n"
                        f"Send URL matching:\n<code>{offer['starting_link']}?clickid=YOUR_ID</code>\n\n"
                        f"Postbacks: {offer['postback_count']}\n"
                        f"Delays: {', '.join(str(d) + 's' for d in offer['delays'])}"
                    )
                    users_collection.update_one({'_id': user_id}, {'$set': {'current_mode': 'offer_mode', 'current_offer_id': offer_id}})
                else:
                    send_message(user_id, "❌ Invalid offer ID")
            ...
            # Offer create
                ...
                send_message(
                    ...
                    "3 postbacks: <code>Premium|https://premium.com|https://premium.com?tid=$clickid|https://track.com?id=$clickid|https://log.com?id=$clickid|5|10|8</code>\n\n"
            ...
            # Offer list
                ...
                if offers:
                    text = "<b>📋 ALL OFFERS</b>\n\n"
                    for i, offer in enumerate(offers, 1):
                        status = "✅ Enabled" if offer.get('enabled', True) else "❌ Disabled"
                        text += (f"<b>{i}. {offer['name']}</b>\n"
                                f"   ID: <code>{offer['_id']}</code>\n"
                                f"   Starting Link: {offer['starting_link'][:50]}...\n"
                                f"   Postbacks: {offer['postback_count']}\n"
                                f"   Status: {status}\n"
                                f"   Submissions: {offer.get('total_submissions', 0)}\n\n")
                    send_message(user_id, text[:4000], reply_markup=manage_offers_keyboard())
                else:
                    send_message(user_id, "📭 No offers yet.", reply_markup=manage_offers_keyboard())
            ...
            # Offer edit
                ...
                if offers:
                    text = "<b>✏️ Edit Offer</b>\n\n"
                    text += "Send offer ID and field to edit:\n"
                    text += "<code>OFFER_ID|field|new_value</code>\n\n"
                    text += "<b>Fields:</b> name, starting_link, enabled\n\n"
                    text += "<b>Available Offers:</b>\n"
                    for i, offer in enumerate(offers, 1):
                        text += f"{i}. <code>{offer['_id']}</code> - {offer['name']}\n"
                    send_message(user_id, text[:4000])
                    users_collection.update_one({'_id': user_id}, {'$set': {'current_mode': 'offer_edit_mode'}})
                else:
                    send_message(user_id, "📭 No offers to edit.", reply_markup=manage_offers_keyboard())
            ...
            # Offer delete
                ...
                if offers:
                    text = "<b>🗑️ Delete Offer</b>\n\n"
                    text += "Send the offer ID to delete:\n\n"
                    text += "<b>Available Offers:</b>\n"
                    for i, offer in enumerate(offers, 1):
                        text += f"{i}. <code>{offer['_id']}</code> - {offer['name']}\n"
                    send_message(user_id, text[:4000])
                    ...
                    send_message(user_id, "📭 No offers to delete.", reply_markup=manage_offers_keyboard())
