    ...
    try:
        ...
        if 'message' in update:
            ...
            # Handle offer edit mode
                ...
                try:
                    if '|' not in text:
                        send_message(chat_id, "❌ Invalid format. Use: <code>OFFER_ID|field|value</code>", reply_markup=admin_keyboard())
                        return 'ok', 200
                    ...
                    offer = get_offer(offer_id)
                    if not offer:
                        send_message(chat_id, f"❌ Offer ID not found: <code>{offer_id}</code>", reply_markup=admin_keyboard())
                        users_collection.update_one({'_id': user_id}, {'$set': {'current_mode': None}})
                        return 'ok', 200
                    ...
                    if field == 'name':
                        ...
                        updates['enabled'] = value.lower() in ['true', 'yes', '1', 'enable']
                    else:
                        send_message(chat_id, f"❌ Invalid field: {field}. Available: name, enabled", reply_markup=admin_keyboard())
                        users_collection.update_one({'_id': user_id}, {'$set': {'current_mode': None}})
                        return 'ok', 200
            ...
            # Handle offer delete mode
                ...
                try:
                    ...
                    send_message(chat_id, f"🗑️ {message}\n\n<b>Deleted Offer:</b> {offer['name']}", reply_markup=admin_keyboard())
        ...
        # Handle callback queries
            ...
            # Offer list/view
                ...
                if offers:
                    ...
                    for i, offer in enumerate(offers, 1):
                        ...
                        text += (f"<b>{i}. {offer['name']}</b> {status}\n"
                                f"   <b>ID:</b> <code>{str(offer['_id'])}</code>\n"
                                f"   <b>Postbacks:</b> {offer['postback_count']}\n"
                                f"   <b>Submissions:</b> {offer.get('total_submissions', 0)}\n"
                                f"   <b>Success:</b> {offer.get('success_count', 0)}\n\n")
                    send_message(user_id, text[:4000], reply_markup=manage_offers_keyboard())
                else:
                    send_message(user_id, "📭 No offers created yet.", reply_markup=manage_offers_keyboard())
            ...
            # Offer edit
                ...
                if offers:
                    ...
                    text += "Copy the <b>Offer ID</b> and send in format:\n"
                    text += "<code>OFFER_ID|field|value</code>\n\n"
                    text += "<b>Fields:</b> name, postbacks, delays, enabled\n\n"
                    text += "<b>Example:</b> <code>64a7f1b2c3d4e5f6g7h8i9j0|name|New Name</code>\n\n"
                    text += "<b>Offers:</b>\n"
            ...
            # Offer delete
                ...
                if offers:
                    ...
                    text += "⚠️ <b>WARNING:</b> Deletion is permanent!\n\n"
                    text += "Send the <b>Offer ID</b> to delete:\n"
                    text += "<code>OFFER_ID</code>\n\n"
                    text += "<b>Offers:</b>\n"
                    for i, offer in enumerate(offers[:10], 1):
                        text += (f"<b>{i}. {offer['name']}</b>\n"
                                f"   <b>ID:</b> <code>{str(offer['_id'])}</code>\n\n")
